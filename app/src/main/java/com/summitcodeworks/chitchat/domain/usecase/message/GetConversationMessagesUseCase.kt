...
class GetConversationMessagesUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        return try {
            val result = messageRepository.getConversationMessages(token, userId, page, size)
    ...
    fun observeLocalMessages(senderId: Long, receiverId: Long): Flow<List<Message>> {
        return messageRepository.getLocalConversationMessages(senderId, receiverId)
            .map { entities ->
                entities.map { entity ->
                    Message(
                        id = entity.id,
